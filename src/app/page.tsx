...
const tableMetrics = [
  { name: "Sales", value: "$232,628.52", isBold: true, color: "text-zinc-100" },
  ...
  { name: "Shipping costs", value: "-$4,044.57", indent: true, color: "text-zinc-400" },
  { name: "Refund cost", value: "-$5,820.42", indent: true, color: "text-zinc-400" },
  { name: "Amazon fees", value: "-$92,652.70", indent: true, color: "text-zinc-400" },
  { name: "Cost of goods", value: "-$35,280.41", indent: true, color: "text-zinc-400" },
  ...
  { name: "Estimated payout", value: "$127,375.77", isBold: true, color: "text-zinc-100" },
  ...
  { name: "Sessions", value: "43,748", color: "text-zinc-300" },
  { name: "Unit session percentage", value: "54.52%", color: "text-zinc-300" },
];
...
export default function Home() {
  ...
  return (
    <div className="flex min-h-screen bg-zinc-950 text-zinc-100 font-sans antialiased">
      {/* --- SIDEBAR --- */}
      <aside className="w-64 shrink-0 border-r border-zinc-900 bg-zinc-950 flex flex-col justify-between">
        <div>
          {/* Logo */}
          ...
          <nav className="p-4 flex flex-col gap-1.5">
            ...
            <div>
              <button
                ...
                className="w-full flex items-center justify-between px-3 py-2 rounded-lg text-sm font-medium text-zinc-400 hover:text-zinc-200 hover:bg-zinc-900/50 transition-all"
              >
              ...
              {/* Sub-menu items */}
              {profitMenuOpen && (
                <div className="mt-1 pl-4 flex flex-col gap-0.5 border-l border-zinc-900 ml-5">
                  {[
                    "Dashboard",
                    "Products",
                    "Shipping Costs",
        ...
        <div className="p-4 border-t border-zinc-900 flex flex-col gap-2">
          ...
          <div className="flex items-center justify-between border-t border-zinc-900/80 pt-2 px-1">
            <button className="p-1.5 rounded-lg text-zinc-500 hover:text-zinc-200 hover:bg-zinc-900 transition-colors">
            ...
            <div className="flex gap-0.5 bg-zinc-900 border border-zinc-800 rounded-lg p-0.5">
              <button className="p-1 rounded bg-zinc-800 text-amber-400">
      ...
      <main className="flex-1 flex flex-col overflow-y-auto bg-zinc-950">
        {/* --- HEADER --- */}
        <header className="h-16 flex items-center justify-between px-8 border-b border-zinc-900 bg-zinc-950/80 backdrop-blur sticky top-0 z-50">
          <div className="flex items-center gap-3">
            <h1 className="text-xl font-bold tracking-tight text-zinc-100">Dashboard</h1>
            <div className="flex items-center gap-1 bg-zinc-900 border border-zinc-800 rounded-lg p-0.5 text-xs text-zinc-400">
              <button className="px-2.5 py-1 rounded bg-zinc-800 text-zinc-100 font-medium shadow-sm">Tiles</button>
              <button className="px-2.5 py-1 rounded hover:text-zinc-200">Chart</button>
              <button className="px-2.5 py-1 rounded hover:text-zinc-200">P&L</button>
              <button className="px-2.5 py-1 rounded hover:text-zinc-200">Map</button>
              <button className="px-2.5 py-1 rounded hover:text-zinc-200">Trends</button>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="relative w-48">
              ...
              <Input
                type="text"
                placeholder="Search..."
                className="pl-8 h-9 text-xs bg-zinc-900/60 border-zinc-800 text-zinc-300 placeholder-zinc-500 focus-visible:ring-amber-500/50"
              />
            </div>

            {/* Filters */}
            <Select defaultValue="all">
              <SelectTrigger className="w-36 h-9 text-xs bg-zinc-900/60 border-zinc-800 focus:ring-amber-500/50">
            ...
            <Select defaultValue="usd">
              <SelectTrigger className="w-24 h-9 text-xs bg-zinc-900/60 border-zinc-800 focus:ring-amber-500/50">
            ...
            <button className="relative p-2 rounded-lg border border-zinc-900 hover:border-zinc-800 bg-zinc-900/40 hover:bg-zinc-900 transition-all">
              <Bell className="size-4 text-zinc-400 hover:text-zinc-200" />
            ...
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">
                {tilesData.map((tile) => (
                  <Card
                    key={tile.title}
                    className={`overflow-hidden border bg-gradient-to-b ${tile.color} transition-all duration-300 hover:scale-[1.02] hover:shadow-lg hover:shadow-zinc-950/50`}
                  >
                    {/* Tile Header */}
                    <div className={`px-4 py-3 flex flex-col gap-0.5 border-b border-zinc-900/60 ${tile.headerBg}`}>
                      <span className="font-semibold text-sm tracking-tight text-zinc-100">{tile.title}</span>
                    ...
                    <CardContent className="p-4 flex flex-col gap-4">
                      ...
                      {/* Grid of metrics */}
                      <div className="grid grid-cols-2 gap-x-2 gap-y-3 pt-3 border-t border-zinc-900/40">
                        <div>
                      ...
                      {/* Est. Payout */}
                      <div className="pt-3 border-t border-zinc-900/40 flex items-center justify-between text-xs">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <Card className="lg:col-span-2 bg-zinc-900/30 border-zinc-900/80 backdrop-blur-sm shadow-xl p-6 flex flex-col gap-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <h3 className="text-base font-bold text-zinc-200">PnL & Advertising Chart</h3>
                ...
                <Card className="bg-zinc-900/30 border-zinc-900/80 backdrop-blur-sm shadow-xl p-5 flex flex-col justify-between">
                  <div>
                    <div className="flex items-center justify-between pb-3 border-b border-zinc-900/80">
                      <h3 className="text-sm font-bold text-zinc-200 uppercase tracking-wider">Metrics Breakdown</h3>
                      <div className="text-[10px] bg-zinc-800 text-zinc-300 font-semibold px-2 py-0.5 rounded border border-zinc-700">
                        Last 12 Months
                      </div>
                    </div>
                      ...
                      <Table>
                        <TableBody>
                          {tableMetrics.map((metric, idx) => (
                            <TableRow
                              key={idx}
                              className="hover:bg-zinc-900/30 border-b border-zinc-900/40"
                            >
                              <TableCell className="py-2.5 px-1 text-xs">
                                <span
                                  className={`${
                                    metric.indent ? "pl-4 text-zinc-500" : "font-semibold text-zinc-300"
                                  }`}
                                >
                                  {metric.name}
                                </span>
                              ...
                              <TableCell className="py-2.5 px-1 text-right text-xs">
                                <span className={metric.color || "text-zinc-200"}>
                                  {metric.value}
                                </span>
                              </TableCell>
                            </TableRow>
                  ...
                  {/* Summary Note */}
                  <div className="mt-4 pt-3 border-t border-zinc-900/80 flex items-center justify-between text-[10px] text-zinc-500">
